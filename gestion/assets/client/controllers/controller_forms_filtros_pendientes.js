document.addEventListener('DOMContentLoaded', () => {
    const btnFiltros = document.getElementById('btnFiltrosPendientes');
    const modalFiltros = document.getElementById('modalFiltrosPendientes');
    const cerrarFiltrosBtn = document.getElementById('cerrarFiltrosBtnPendientes');
    const modalBackground = modalFiltros?.querySelector('.modal-background');
    const aplicarFiltrosBtn = document.getElementById('aplicarFiltrosBtnPendientes');
    const cancelarFiltrosBtn = document.getElementById('cancelarFiltrosBtnPendientes');
...
export async function aplicarFiltrosPendientes() {
    const empresa = document.getElementById('empresaFiltroPendientes')?.value.toLowerCase() || '';
    const empresaDestino = document.getElementById('empresa_destinoFiltroPendientes')?.value.toLowerCase() || '';
    const fechaInicio = document.getElementById('fechaInicioFiltroPendientes')?.value || '';
    const fechaFin = document.getElementById('fechaFinFiltroPendientes')?.value || '';
    ...
    try {
        const response = await fetch('../../server/backend/modules/fetch-remitos-pendientes-filtros.php', {
            method: 'POST',
            body: formData,
        });
        ...
        if (data.success) {
            const tbody = document.querySelector('#remitosTablePendientes');
            if (tbody) {
                tbody.innerHTML = ''; // Limpiar la tabla
                if (data.data.length > 0) {
                    console.log('Datos de remitos recibidos:', data.data); // <-- Log para ver los datos
                    renderRemitosTablePendientes(data, tbody); // Renderizar los resultados
                } else {
...
export function renderRemitosTablePendientes(data, tbody) {
